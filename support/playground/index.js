...
function getOptions(container) {
  ...
  try {
    options.connections = JSON.parse($('[name="connections"]').val());
  } catch (e) {}
...
function removeDefaultOptions(options) {
  ...
  removeKeys(options, ['closable', 'focusInput', 'gravatar', 'rememberLastLogin'], function(value) {
    return value === true;
  });
  ...
  removeKeys(
    options,
    ['container', 'dict', 'icon', 'primaryColor', 'authParams', 'callbackURL', 'defaultLocation'],
    function(value) {
      return !value;
    }
  );
...
function tryDisplayAuthenticatedFeedback() {
  ...
  var response = lock.parseHash(window.location.hash, function(err, authResult) {
    if (err) {
      return console.error(err);
    }
    renderAuthenticationSuccessMessage(authResult);
  });
}
